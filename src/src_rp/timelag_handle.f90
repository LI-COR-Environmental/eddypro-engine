module mo_timelag_handle

    implicit none

    private

    public :: CalculateTrend ! linear trend of time series
    public :: CovarianceW    ! covariance of specific timelag
    public :: CovMax         ! time lag by maximum covariance within min/max
    public :: Detrend        ! detrend variable given trend from CalculateTrend
    public :: TimeLagHandle  ! calculate time lags
    public :: VariableLinearDetrending     ! detrend using CalculateTrend and Detrend
    public :: VariableStochasticDetrending ! stochastic detrending
...
contains
    ...
    ! \todo
    !***************************************************************************
    subroutine TimeLagHandle(TlagMeth, Set, ActTLag, TLag, &
        ...
        !> in/out variables
        character(len=*),           intent(in)    :: TlagMeth
        real(dbl), dimension(:, :), intent(inout) :: Set
        real(dbl), dimension(:),    intent(out)   :: ActTLag
        real(dbl), dimension(:),    intent(out)   :: TLag
        logical,   dimension(:),    intent(out)   :: DefTlagUsed
        logical,                    intent(in)    :: InTimelagOpt
        real(dbl), dimension(:, :), intent(out), optional :: CorrSet

        !> local variables
        integer :: i
        integer :: j
        integer, dimension(size(Set, 2)) :: def_rl
        integer, dimension(size(Set, 2)) :: min_rl
        integer, dimension(size(Set, 2)) :: max_rl
        real(dbl), dimension(size(Set, 1)) :: ColW
        real(dbl), dimension(size(Set, 1)) :: ColH2O
        real(dbl), dimension(size(Set, 1)) :: ColTC
        real(dbl), dimension(size(Set, 1)) :: FirstCol
        real(dbl), dimension(size(Set, 1)) :: SecondCol
        real(dbl), dimension(size(Set, 1), size(Set, 2)) :: TmpSet
        real(dbl), dimension(size(Set, 1)) :: CrossCorr ! cross-correlations
