...
! \todo
!***************************************************************************
 subroutine FirstFileOfCurrentPeriod(InitialTimestamp, FinalTimestamp, FileList, &
        NumRawFiles, LatestRawFile, NextRawFile, skip_period)
    ...
    !> Looks for the first file containing data relevant to the current period, if any
    NextRawFile = LatestRawFile
    do i = LatestRawFile, NumRawFiles
        if(FileIsRelevantToCurrentPeriod(FileList(i)%name, InitialTimestamp, FinalTimestamp)) then
            NextRawFile = i
            return
        end if

        !> Check if the cycle is moving away from InitialTimestamp. In that case exit loop
        !> This should keep the loop always very short
...
!***************************************************************************
!
! \brief       Detect whether a given raw file is relevant to the given time period
! \author      Gerardo Fratini
! \note
! \sa
! \bug
...
! \todo
!***************************************************************************
logical function FileIsRelevantToCurrentPeriod(Filename, InitialTimestamp, FinalTimestamp)
    use m_common_global_var
    implicit none
    !> In/out variables
    ...
    if (Timestamp == InitialTimestamp .or. &
        (Timestamp < InitialTimestamp .and. Timestamp + DatafileDateStep > InitialTimestamp) .or. &
