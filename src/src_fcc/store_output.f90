...
! \todo
!***************************************************************************
subroutine StoreOutput(lEx, dataline_eu, dataline_rich, dataline_md)
    use m_fx_global_var
    implicit none
    !> in/out variables
    Type(ExType), intent(in) :: lEx
    character(*), intent(out) :: dataline_eu
    character(*), intent(out) :: dataline_rich
    character(*), intent(out) :: dataline_md
    !> local variables
    integer :: var
    integer :: i
    integer :: gas
    ...
    if (EddyProProj%out_full) then
        call clearstr(dataline_rich)
        !> Preliminary file and timestamp information
        call AddDatum(dataline_rich, lEx%fname(1:len_trim(lEx%fname)), separator)
        call AddDatum(dataline_rich, lEx%date(1:10), separator)
        call AddDatum(dataline_rich, lEx%time(1:5), separator)
        call WriteDatumFloat(float_doy, datum, EddyProProj%err_label)
        call stripstr(datum)  !< Added to fix a strange behaviour of
        call AddDatum(dataline_rich, datum(1:index(datum, '.') + 3), separator)
        write(datum, *) lEx%daytime
        call AddDatum(dataline_rich, datum, separator)
        ...
        elseif(EddyProProj%fix_out_format) then
            call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
        end if
        ...
        do gas = co2, n2o
            ...
            elseif(EddyProProj%fix_out_format) then
                ...
                call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
                call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
                call AddDatum(dataline_rich, 'F', separator)
            end if
        end do
        ...
        if (Meth%foot /= 'none') then
            call AddDatum(dataline_rich, foot_model_used(1:len_trim(foot_model_used)), separator)
            ...
            call AddDatum(dataline_rich, datum, separator)
        elseif(EddyProProj%fix_out_format) then
            call AddDatum(dataline_rich, 'none', separator)
            call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
            call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
            call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
            call AddDatum(dataline_rich, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
        ...
        if (Diag7200%present) then
            do i = 1, 9
                call WriteDatumInt(lEx%licor_flags(i), datum, EddyProProj%err_label)
                call AddDatum(dataline_rich, datum, separator)
            end do
        elseif(EddyProProj%fix_out_format) then
            do i = 1, 9
...
!        end if
    ...
    if (EddyProProj%out_md) then
        call clearstr(dataline_md)
        !> Preliminary timestmap information
        write(datum, *) lEx%fname(1:len_trim(lEx%fname))
        call AddDatum(dataline_md, datum, separator)
        write(datum, *) lEx%date(1:10)
        call AddDatum(dataline_md, datum, separator)
        write(datum, *) lEx%time(1:5)
        call AddDatum(dataline_md, datum, separator)
