...
! \todo
!***************************************************************************
subroutine OutputSpectralAssessmentResults(out_assessment, out_ensemble, nbins)
    use m_fx_global_var
    implicit none
    !> in/out variables
    integer, intent(in) :: nbins
    logical, intent(in) :: out_assessment
    logical, intent(in) :: out_ensemble
    !> local variables
    ...
    if (out_assessment) then
        ...
        write(udf,'(a)') 'RH/fc_exponential_fit_parameters_for_water_vapour_spectral_corrections'
        write(udf,'(a)') '-----------------------------------'
        write(udf,'(a)') '         exp1         exp2         exp3'
        write(udf,'(3(f13.6))') RegPar(dum, dum)%e1, RegPar(dum, dum)%e2, RegPar(dum, dum)%e3
        write(udf,'(a)') ''
        write(udf,'(a)') ''

        write(udf, '(a)') 'High-pass_correction_factor_model_parameters'
        write(udf, '(a)') 'Model: CF = [c1 * u / (c2 + f_co) + 1] after_Ibrom_et_al_(2007_AFM)'
        write(udf, '(a)') '-------------------------------------------------------------------'
        write(udf, '(a)') '                   c1          c2'
    ...
    if (out_assessment .or. out_ensemble) then
        !> Average H2O spectra, sorted in RH classes, and predicted spectra (RHS of
        ...
        do cls = RH10, RH90
            call WriteDatumInt(MeanBinSpec(1, cls)%cnt(h2o), datum, EddyProProj%err_label)
        ...
        write(udf,'(a)') 'nat_freq,avrg_sp(T),avrg_sp(h2o),pred_sp(h2o)&
                        &,avrg_sp(T),avrg_sp(h2o),pred_sp(h2o),avrg_sp(T),avrg_sp(h2o),pred_sp(h2o)&
                        &,avrg_sp(T),avrg_sp(h2o),pred_sp(h2o),avrg_sp(T),avrg_sp(h2o),pred_sp(h2o)&
                        &,avrg_sp(T),avrg_sp(h2o),pred_sp(h2o),avrg_sp(T),avrg_sp(h2o),pred_sp(h2o)&
                        &,avrg_sp(T),avrg_sp(h2o),pred_sp(h2o),avrg_sp(T),avrg_sp(h2o),pred_sp(h2o)'
        ...
        do i = 1, nbins - 1
            ...
            if (MeanBinSpec(i, goodj)%fn(h2o) /= error) then
                call WriteDatumFloat(MeanBinSpec(i, goodj)%fn(h2o), datum, EddyProProj%err_label)
                call AddDatum(dataline, datum, separator)
                do cls = RH10, RH90
                    if (MeanBinSpecAvailable(cls, h2o)) then
                        call WriteDatumFloat(MeanBinSpec(i, goodj)%fn(h2o) * MeanBinSpec(i, cls)%ts(h2o), &
                            datum, EddyProProj%err_label)
                        call AddDatum(dataline, datum, separator)
                        call WriteDatumFloat(MeanBinSpec(i, goodj)%fn(h2o) * MeanBinSpec(i, cls)%of(h2o), &
                            datum, EddyProProj%err_label)
                        call AddDatum(dataline, datum, separator)
                        call WriteDatumFloat(RegPar(h2o, cls)%Fn &
                            * (1d0 / (1d0 + (MeanBinSpec(i, goodj)%fn(h2o) / RegPar(h2o, cls)%fc)**2 )) &
                            * MeanBinSpec(i, cls)%ts(h2o) * MeanBinSpec(i, goodj)%fn(h2o), datum, EddyProProj%err_label)
                        call AddDatum(dataline, datum, separator)
                    else
    ...
    !FOR MONTH-SORTING   &avrg_sp(T),avrg_sp(' // g4lab(1:g4l) // '),pred_sp(' // g4lab(1:g4l) // ')'
        ...
        !> If none, it means all spectra are -9999, so -9999 is written.
        !> Basically, run failed.
        call GetFnIndex(MeanBinSpec, size(MeanBinSpec, 1), size(MeanBinSpec, 2), goodj, pick)
    ...
    if (out_ensemble) then
        ...
        do cls = 1, 8
            do gas = w_ts, w_gas4
                call WriteDatumInt(MeanBinCosp(1, cls)%cnt(gas), datum, EddyProProj%err_label)
                call AddDatum(dataline, 'n_=_' // datum(1:len_trim(datum)), separator)
            end do
        end do
        write(udf,'(a)') dataline(1:len_trim(dataline) - 1)
        !> Add header piece
        write(udf,'(a)') &
            'nat_freq,avrg_cosp(w/T),avrg_cosp(w/co2),avrg_cosp(w/h2o),avrg_cosp(w/ch4),avrg_cosp(w/' // g4lab(1:g4l) //')&
                    &,avrg_cosp(w/T),avrg_cosp(w/co2),avrg_cosp(w/h2o),avrg_cosp(w/ch4),avrg_cosp(w/' // g4lab(1:g4l) //')&
                    &,avrg_cosp(w/T),avrg_cosp(w/co2),avrg_cosp(w/h2o),avrg_cosp(w/ch4),avrg_cosp(w/' // g4lab(1:g4l) //')&
                    &,avrg_cosp(w/T),avrg_cosp(w/co2),avrg_cosp(w/h2o),avrg_cosp(w/ch4),avrg_cosp(w/' // g4lab(1:g4l) //')&
                    &,avrg_cosp(w/T),avrg_cosp(w/co2),avrg_cosp(w/h2o),avrg_cosp(w/ch4),avrg_cosp(w/' // g4lab(1:g4l) //')&
                    &,avrg_cosp(w/T),avrg_cosp(w/co2),avrg_cosp(w/h2o),avrg_cosp(w/ch4),avrg_cosp(w/' // g4lab(1:g4l) //')&
        ...
        !> If none, it means all spectra are -9999, so -9999 is written.
        !> Basically, run failed.
        call GetFnIndex(MeanBinCosp, size(MeanBinCosp, 1), size(MeanBinCosp, 2), goodj, pick)

        do i = 1, nbins - 1
            call clearstr(dataline)
            if (MeanBinCosp(i, goodj)%fn(pick) /= error) then
                call WriteDatumFloat(MeanBinCosp(i, goodj)%fn(pick), datum, EddyProProj%err_label)
                call AddDatum(dataline, datum, separator)
                do cls = 1, 8
                    do gas = w_ts, w_gas4
                        if (MeanBinCospAvailable(cls, gas))then
                            call WriteDatumFloat(MeanBinCosp(i, goodj)%fn(pick) * MeanBinCosp(i, cls)%of(gas), &
                                datum, EddyProProj%err_label)
                            call AddDatum(dataline, datum, separator)
                        else
                            call AddDatum(dataline, EddyProProj%err_label(1:len_trim(EddyProProj%err_label)), separator)
        ...
        !> Stability-sorted cospectra and models, only if at least one fit succeed
        proceed = .false.
        do gas = w_ts, w_co2
